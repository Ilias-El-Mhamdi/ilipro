...
export function CompanySection({section, user, isAdmin = true}: Props) {
    return (
        <div className="mb-10">
            ...
            <ProjectsSubSection section={section}/>
        </div>
    );
}
    ...
    return (
        <div className="flex items-center gap-2 mb-5 pb-2 border-b border-gray-800">
            {isAdmin ? (
                ...
                >
                    {name}
                    <svg xmlns="http://www.w3.org/2000/svg" className="w-3.5 h-3.5 text-gray-600"
                         viewBox="0 0 24 24"
...
function ProjectsSubSection({section}: { section: UserCompanySection; }) {
    return (
        <div>
            ...
            {section.projects.length === 0 ? (
                ...
                <div className="flex flex-col gap-4">
                    {section.projects.map((project) => (
                        <ProjectRow key={project.id} project={project} members={section.members}/>
                    ))}
                </div>
            )}
        </div>
